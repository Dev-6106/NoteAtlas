...
import { renameFolder } from '@/store/folderSlice';
...
export default function RenameFolderModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (folderName.trim() && folderName.trim() !== currentName) {
      await dispatch(renameFolder({ id: folderId, name: folderName.trim() }));
      onClose();
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-default)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--primary-glow)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Edit2 size={18} style={{ color: 'var(--primary-brand)' }} />
            </div>
            <h2 style={{ fontSize: 18, fontWeight: 600, color: 'var(--text-1)', fontFamily: 'var(--font-sans)' }}>Rename Folder</h2>
          </div>
